...
{addData && addData.files.map((val,index)=>{
  return <div className='w-[10rem] p-1.5 border relative' key={index}>
    
      <img src={addData.files.length>0 ? val.fieldname?`${import.meta.env.VITE_BACKEND_URL}/${val.path}`: URL.createObjectURL(val):""} 
